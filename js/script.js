...
// Helper: compatibilità tra server che restituisce `id` e server Mongo/NeDB che restituisce `_id`
function getId(item) {
        ...
        <div class="item-row">
            <div class="item-info" style="flex:1">
                ...
                <div style="display:flex; align-items:center; gap:10px; margin-top:5px;">
                    <input type="range" value="${item.rimasto}" onchange="updFrigo('${id}', this.value)">
                    <span style="font-size:12px; width:30px;">${item.rimasto}%</span>
                </div>
            </div>
            <button onclick="delFrigo('${id}')" class="st-btn" style="border:none; font-size:1.2rem; color:#ff4b4b;">✕</button>
        </div>`;
    });
}
...
async function updFrigo(id, val) {
    await fetch(`${API}/frigo/${encodeURIComponent(id)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
async function delFrigo(id) {
    await fetch(`${API}/frigo/${encodeURIComponent(id)}`, { method: 'DELETE' });
    loadFrigo();
}
...
async function loadCarrello() {
    ...
    data.forEach(item => {
        ...
        <div class="item-row" style="border-left: 3px solid #00d4b1;">
            ...
            <button onclick="delCarrello('${id}')" class="st-btn" style="border:none; color:#00d4b1;">✔</button>
        </div>`;
    });
}
...
async function delCarrello(id) {
    await fetch(`${API}/carrello/${encodeURIComponent(id)}`, { method: 'DELETE' });
    loadCarrello();
}
